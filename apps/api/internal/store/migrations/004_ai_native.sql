...
JOIN capabilities c ON c.code IN (
  'employee.read', 'employee.write', 'attendance.manage', 'message.manage',
  'rag.search', 'learning.view', 'learning.manage',
  'agent.execute_read', 'visual_copilot.use'
)
...
FROM roles r
JOIN capabilities c ON c.code IN (
  'employee.read', 'rag.search', 'learning.view', 'agent.execute_read', 'visual_copilot.use'
)
...
VALUES
  ('00000000-0000-0000-0000-000000000901', 'upload', '入职制度样例', 'seed://onboarding', 'active', '00000000-0000-0000-0000-000000000301')
ON CONFLICT DO NOTHING;
...
VALUES
  ('00000000-0000-0000-0000-000000000911', '00000000-0000-0000-0000-000000000901', '新员工入职学习指南', 'v1', 'published', 'official', 'normal',
   '新员工应在入职 7 天内完成公司制度、信息安全、考勤规则和组织协作课程。直属经理需要在 30 天内完成一次复盘。',
   encode(digest('新员工入职学习指南', 'sha256'), 'hex'), now(), '00000000-0000-0000-0000-000000000301')
ON CONFLICT DO NOTHING;

INSERT INTO rag_document_scopes (document_id, scope_type, scope_id, include_descendants)
SELECT '00000000-0000-0000-0000-000000000911', 'global', NULL, true
WHERE NOT EXISTS (
  SELECT 1 FROM rag_document_scopes
  WHERE document_id = '00000000-0000-0000-0000-000000000911' AND scope_type = 'global'
);

INSERT INTO rag_chunks (id, document_id, chunk_index, title, content, content_hash, sensitivity)
VALUES
  ('00000000-0000-0000-0000-000000000921', '00000000-0000-0000-0000-000000000911', 0, '入职 7/30 天要求',
   '新员工应在入职 7 天内完成公司制度、信息安全、考勤规则和组织协作课程。直属经理需要在 30 天内完成一次复盘。',
   encode(digest('入职 7/30 天要求', 'sha256'), 'hex'), 'normal')
ON CONFLICT DO NOTHING;

INSERT INTO rag_embeddings (chunk_id, provider, model, dimensions, embedding)
VALUES
  ('00000000-0000-0000-0000-000000000921', 'fake', 'deterministic-v1', 8, '[0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80]')
ON CONFLICT DO NOTHING;

INSERT INTO learning_courses (id, title, description, status, scope_type, created_by_user_id)
VALUES
  ('00000000-0000-0000-0000-000000000931', '新员工 30 天上手计划', '面向新入职员工的制度、工具、协作和复盘课程。', 'published', 'global', '00000000-0000-0000-0000-000000000301')
ON CONFLICT DO NOTHING;

INSERT INTO learning_lessons (id, course_id, title, content, sort_order, rag_document_id)
VALUES
  ('00000000-0000-0000-0000-000000000932', '00000000-0000-0000-0000-000000000931', '制度与考勤', '学习公司制度、考勤规则和常见异常处理方式。', 1, '00000000-0000-0000-0000-000000000911'),
  ('00000000-0000-0000-0000-000000000933', '00000000-0000-0000-0000-000000000931', '组织协作', '理解集团、法人实体、组织单元和项目组的协作方式。', 2, '00000000-0000-0000-0000-000000000911')
ON CONFLICT DO NOTHING;

INSERT INTO learning_enrollments (employee_id, course_id, status, due_date, created_by_user_id)
VALUES
  ('00000000-0000-0000-0000-000000000402', '00000000-0000-0000-0000-000000000931', 'assigned', current_date + 30, '00000000-0000-0000-0000-000000000301')
ON CONFLICT DO NOTHING;
